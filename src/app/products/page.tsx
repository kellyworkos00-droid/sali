...
export default async function ProductsPage({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      {/* Header Section */}
      <div className="bg-gradient-to-r from-brand-200 to-brand-200 text-white py-16">
      ...
      <div className="container mx-auto px-4 py-8">
          ...
          <div className="flex flex-wrap gap-3">
            <Link
              ...
              className={`px-6 py-3 rounded-xl transition-all font-semibold flex items-center gap-2 ${
                !category
                  ? "bg-gradient-to-r from-brand-200 to-brand-200 text-white shadow-lg shadow-brand-200/30 scale-105"
                  : "bg-gray-100 text-gray-700 hover:bg-blue-50 hover:text-brand-200 border-2 border-gray-200"
              }`}
            >
              <Package size={18} />
              All Products
        ...
        <div className="mb-6">
          <p className="text-gray-600 text-lg">
            <span className="font-semibold text-brand-200">{products.length}</span> {category ? `${category}` : 'products'} found
          </p>
        </div>
        ...
        {products.length > 0 ? (
          ...
          <div className="text-center py-20 bg-white rounded-2xl shadow-lg">
            ...
            <Link href="/products" className="text-brand-200 hover:underline mt-4 inline-block">
              View all products
            </Link>
          </div>
        )}
